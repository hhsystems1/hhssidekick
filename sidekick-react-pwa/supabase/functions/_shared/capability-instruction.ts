...
export function normalizeCapabilityInstruction(
  ...
  if (!trimmed) {
    throw new Error('Add an instruction before queueing the job.');
  }
  ...
  switch (action) {
    case 'github.repo.read':
      return normalizeGitHubRead(trimmed, fields);
    case 'github.repo.write':
      return normalizeGitHubWrite(trimmed, fields);
    case 'gmail.send':
      return normalizeGmail(trimmed, fields);
    case 'calendar.create':
      return normalizeCalendar(trimmed, fields, fallbackTimeZone);
    case 'rivryn.project.read':
      return normalizeRivryn(trimmed, fields);
    case 'code.exec':
      return normalizeCodeExec(trimmed, fields);
    default:
...
function normalizeGitHubRead(instruction: string, fields: KeyValueMap) {
  ...
  return compactObject({ owner, repo, path, ref });
}
...
function normalizeGitHubWrite(instruction: string, fields: KeyValueMap) {
  ...
  return compactObject({ owner, repo, path, message, content, branch });
}

function normalizeGmail(instruction: string, fields: KeyValueMap) {
  const to =
    firstField(fields, ['to', 'recipient']) ||
    instruction.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i)?.[0];
  const subject =
...
function normalizeCalendar(instruction: string, fields: KeyValueMap, fallbackTimeZone: string) {
  ...
  const addMeet =
    parseBoolean(firstField(fields, ['add_meet', 'meet', 'google_meet'])) ||
    /\b(add meet|google meet|meet link|video call)\b/i.test(instruction);
...
function normalizeRivryn(instruction: string, fields: KeyValueMap) {
  ...
  return compactObject({ endpoint, method, body });
}
